...
.libPaths(c("/oak/stanford/groups/longaker/KEBR/Human_Bowel/IBDAFs/r_packages/updated",.libPaths()))
...
markers <- FindAllMmarkers(merged, only.pos = T, logfc.threshold = 0.25)
...
spat<-  readRDS("/oak/stanford/groups/longaker/KEBR/Human_Bowel/Visium/CytoSpace/Longakerfibroticrefspat.rds")
spat <- spat
...
Yapgenes <- Yapgenes[Yapgenes %in% rownames(spat)] #retain YAP genes only in rownames of fibro 
genes.to.keep <- Matrix::rowSums(spat[["Spatial"]]$counts > 0) >= floor(0.1 * ncol(spat[["Spatial"]]$counts))
counts.sub <- spat[["Spatial"]]$counts[genes.to.keep,]
sub <- Yapgenes[Yapgenes %in% rownames(counts.sub)] #Remove genes not in 10% of fibroblasts
spat <- AddModuleScore(spat, features = list(sub), name = "Yapsig") #Get score for YAP targets
#Plot Yap signature
p1 <- SpatialFeaturePlot(spat, features = "Yapsig1", images = "slice19", pt.size.factor = 2.0, stroke = 0.25, max.cutoff = "q95", image.alpha = 0) + 
  theme(legend.position="right") + 
  ggtitle("Outer Region") + theme(plot.title = element_text(hjust = 0.5, size = 15)) + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
p1 <- rotate_image(p1, 118, 1) 
p2 <- SpatialFeaturePlot(spat, features = "Yapsig1", images = "slice9", pt.size.factor = 2.0, stroke = 0.25, max.cutoff = "q95", image.alpha = 0) + 
  theme(legend.position="right") + 
...
spat <- AddModuleScore(spat, features = ecmgenes, name = "ECMgenes")
p3 <- SpatialFeaturePlot(spat, features = "ECMgenes1", images = "slice19", pt.size.factor = 2.0, stroke = 0.25, max.cutoff = "q95", image.alpha = 0) + 
   theme(legend.position="right") + 
  ggtitle("Outer Region") + theme(plot.title = element_text(hjust = 0.5, size = 15)) 
p3 <- rotate_image(p3, 118, 1)
p4 <- SpatialFeaturePlot(spat, features = "ECMgenes1", images = "slice9", pt.size.factor = 2.0, stroke = 0.25, max.cutoff = "q95", image.alpha = 0) + 
  theme(legend.position="right") + 
  ggtitle("Inner Region") + theme(plot.title = element_text(hjust = 0.5, size = 15)) 
p4 <- rotate_image(p4, -153, 0.83)

p1 <- VlnPlot(spat, features = c("ECMgenes1"), group.by = "Domain", cols =c("#ABD9E9", "#FDAE61", "#D7191C") )

p4 + p3 

group1 <- data_df %>% filter(Domain == "BW") %>% pull(expression)
group2 <- data_df %>% filter(Domain == "CF") %>% pull(expression)

# Perform a t-test
t_test_result <- t.test(group1, group2, var.equal = FALSE)  # Welch’s t-test (default)
print(t_test_result$p.value)  # Extract p-value
...
components <- list(c("YAP1", "CTNNB1", "MST1",
                     ...
                     "VIM", "VCL", "VASP"))
spat <- AddModuleScore(spat, features = components, name = "Components")
range = range(spat$Components1)
q = "q95" #set quantile to cut off for spatial 
p5 <- SpatialFeaturePlot(spat, features = "Components1", images = "slice19", pt.size.factor = 2.0, stroke = 0.25, max.cutoff = q, image.alpha = 0) + 
  theme(legend.position="right") + 
  ggtitle("Outer Region") + theme(plot.title = element_text(hjust = 0.5, size = 15)) 
p5 <- rotate_image(p5, 118, 1)
p6 <- SpatialFeaturePlot(spat, features = "Components1", images = "slice9", pt.size.factor = 2.0, stroke = 0.25, max.cutoff = q, image.alpha = 0) + 
   theme(legend.position="right") + 
  ggtitle("Inner Region") + theme(plot.title = element_text(hjust = 0.5, size = 15)) 
...
evels(spat) <- c("Stromal 1", "Stromal 2", "Stromal 3", "Stromal 4")   
DotPlot(spat, features = c("Components1", "Yapsig1", "ECMgenes1"), scale.max = 100, scale.min = 0, cols = "RdBu") + theme(axis.title.x = element_blank(),
...
ggsave("Figures/dotplotspat.jpg", width = 10, height = 4, units = "in")

DotPlot(spat, features = c("ECMgenes1", "Myo1", "Yapsig1"), scale.max = 100, scale.min = 0, cols = "RdBu") + theme(axis.title.x = element_blank(),
                                                                                                                          axis.title.y = element_blank(), 
                                                                                                                          axis.text.x = element_text(size = 15),
                                                                                                                          axis.text.y = element_text(hjust = 1, vjust = 0.5, size = 15), 
                                                                                                                          legend.key.size = unit(0.8, 'cm'), #change legend key size
                                                                                                                          ...
                                                                                                                          legend.title = element_text(size=9), #change legend title font size
                                                                                                                          legend.text = element_text(size=9)) +  #change legend text font size)) +
  scale_x_discrete(labels=c('ECM', "MF", "YAP"))

ggsave("Figures/dotplotspatsigs.svg")
library(RColorBrewer)
FeaturePlot(fibro, features = c("ANT1")) + scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))

#Find myofibroblast genes: 

myo <- list(c("ACTA2", "TAGLN", "MYH11", "FAP", "POSTN", 
              "CTHRC1", "CDH11", "CNN1", "CALDH1"))

remod <- list(c("MMP3", "ACTG2", "NDNF", "CDH11", "COL5A3", "COL7A1", "CXCL14", 
                "CXCL5", "ESM1", "IGHD", "IGHG1", "IL1B", "MAB21L2", "MCAM", 
                "OLFML2A", "P2RY14", "PDE4B", "PRKAR2B", "PSMB2", "RGS5", 
                "RUNX1T1", "TPSAB1", "VWF", "WNT5A", "ZNF117"))
spat <- AddModuleScore(spat, features = remod, name = "Remod")
spat <- AddModuleScore(spat, features = myo, name = "Myo")
p3 <- SpatialFeaturePlot(spat, features = "Myo1", images = "slice19", pt.size.factor = 2.0, stroke = 0, max.cutoff = "q95", image.alpha = 0) + 
  theme(legend.position="right") + 
  ggtitle("Outer Region") + theme(plot.title = element_text(hjust = 0.5, size = 15)) 
p3 <- rotate_image(p3, 118, 1)
p4 <- SpatialFeaturePlot(spat, features = "Myo1", images = "slice9", pt.size.factor = 2.0, stroke = 0, max.cutoff = "q95", image.alpha = 0) + 
  theme(legend.position="right") + 
  ggtitle("Inner Region") + theme(plot.title = element_text(hjust = 0.5, size = 15)) 
p4 <- rotate_image(p4, -153, 0.83)
p3+p4
...
# Assuming you have a Seurat object named 'spat'
# and you want to plot features on slice images

# Define a function to create and customize the SpatialFeaturePlot
create_custom_plot <- function(spat, feature, image, rotation_angle) {
  plot <- SpatialFeaturePlot(spat, features = feature, images = image, pt.size.factor = 2.0, 
...
# Create the plots
p1 <- create_custom_plot(spat, "COL1A1", "slice19", 118)
p2 <- create_custom_plot(spat, "COL1A2", "slice19", 118)
p3 <- create_custom_plot(spat, "COL3A1", "slice19", 118)
p4 <- create_custom_plot(spat, "FN1", "slice19", 118)
p5 <- create_custom_plot(spat, "COL1A1", "slice9", -153)
p6 <- create_custom_plot(spat, "COL1A2", "slice9", -153)
p7 <- create_custom_plot(spat, "COL3A1", "slice9", -153)
p8 <- create_custom_plot(spat, "FN1", "slice9", -153)
...
# Create the plots
p1 <- create_custom_plot(spat, "ACTA2", "slice19", 118)
p2 <- create_custom_plot(spat, "TAGLN", "slice19", 118)
p3 <- create_custom_plot(spat, "MYH11", "slice19", 118)
p4 <- create_custom_plot(spat, "DES", "slice19", 118)
p5 <- create_custom_plot(spat, "ACTA2", "slice9", -153)
p6 <- create_custom_plot(spat, "TAGLN", "slice9", -153)
p7 <- create_custom_plot(spat, "MYH11", "slice9", -153)
p8 <- create_custom_plot(spat, "DES", "slice9", -153)
...
# Create the plots
p1 <- create_custom_plot(spat, "CCN1", "slice19", 118)
p2 <- create_custom_plot(spat, "CCN2", "slice19", 118)
p5 <- create_custom_plot(spat, "CCN1", "slice9", -153)
p6 <- create_custom_plot(spat, "CCN2", "slice9", -153)
